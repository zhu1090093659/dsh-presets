...
本仓同时拥有插件与 [`presets/`](presets/) 下的社区预设目录。预设以 PR 形式投稿，投稿清单与评审规则见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
- **目录内容取决于被钉扎的那份 catalog。** 本仓的 `presets/catalog.json` 是发布源；dsh-web 的市场构建按 submodule 钉扎的提交读取它，因此合并的预设要等该钉扎前移才到达用户。贡献格式与投稿必须附带的证据见 [CONTRIBUTING.md](CONTRIBUTING.md) 与 [presets/README.md](presets/README.md)。
...
- `presets/` —— 创意工坊「预设」类目的发布源（catalog 加每个预设一个目录），由 dsh-web 的市场构建按 submodule 钉扎的提交读取。
