...
This repository owns both the plugin and the community preset catalog in [`presets/`](presets/). Presets are contributed here as pull requests — the submission checklist and the review rules are in [CONTRIBUTING.md](CONTRIBUTING.md).
...
- **The catalog is whatever the pinned catalog lists.** `presets/catalog.json` in this repository is the publishing source; the dsh-web market build reads it from the pinned submodule commit, so a merged preset reaches users when that pin moves. The contribution format and the evidence a submission must carry are in [CONTRIBUTING.md](CONTRIBUTING.md) and [presets/README.md](presets/README.md).
...
- `presets/` — the publishing source for the Workshop's Presets category (catalog plus one directory per preset), read by the dsh-web market build from the pinned submodule commit.
